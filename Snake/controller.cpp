...
void Controller::Select()//选择界面
{
    ...
    std::cout << "炼狱模式";
    SetColor(4);
    SetCursorPosition(6, 27);
    std::cout << "Made By Vijay";
...
{
    mciSendStringW(L"play Sounds/Snake.mp3 repeat", 0, 0, 0);
    system("cls");//清屏
    /*绘制地图*/
    SetColor(3);
...
int Controller::PlayGame()//游戏二级循环
{
    ...
    while (csnake->OverEdge() && csnake->HitItself()) //判断是否撞墙或撞到自身，即是否还有生命
    {
        ...
        {
            mciSendStringW(L"play Sounds/eat.mp3", 0, 0, 0);
            csnake->Move();//蛇增长
        ...
        {
            mciSendStringW(L"play Sounds/time.mp3", 0, 0, 0);
            csnake->Move();
...
void Controller::Game()//游戏一级循环
{
    ...
    while (true)//游戏可视为一个死循环，直到退出游戏时循环结束
    {
        ...
        if (tmp == 1) //返回值为1时重新开始游戏
        {
            system("cls");
            SetColor(7);//设置动画颜色
            StartInterface* ui = new StartInterface();//动态分配一个StartInterface类ui
            ui->Logo();//动画
            delete ui;//释放内存空间
            continue;
        }
...
{
    /*绘制游戏结束界面*/
    mciSendStringW(L"play Sounds/over.mp3", 0, 0, 0);
    Sleep(500);
    SetColor(11);
